...
std::set<Kernel::FT> getZEvents (Polyhedron p, Kernel::FT mindiff) {
    ...
    first = true;
    Kernel::FT prev;
    for (std::set<Kernel::FT>::iterator it = zevents.begin(); it != zevents.end(); ) {
	Kernel::FT cur = *it;
...
pair<Point3d, Point3d> PMRegion::boundingbox() {
    ...
    return pair<Point3d, Point3d>(Point3d(minx, miny, minz), Point3d(maxx, maxy, maxz));
}
...
void PMRegion::analyze() {
    ...
    while (++it != zevents.end()) {
        ...
        int count = 0;
        for (std::vector<pair<Kernel::FT,Kernel::FT> >::iterator ri = ranges.begin(); ri != ranges.end(); ++ri) {
    ...
    int vertices = distance(polyhedron.vertices_begin(), polyhedron.vertices_end());
...
void PMRegion::openscad(string filename) {
    ...
    for (Point_iterator pi = polyhedron.points_begin(); pi != polyhedron.points_end(); ++pi) {
        Kernel::FT z = pi->z();
...
//    for (list<Segment2d>::iterator it = segs.begin(); it != segs.end(); it++) {
//        scad << "line(" << it->source().x() << ", " << it->source().y() << ", "
//                        << it->target().x() << ", " << it->target().y() << ");" << endl;
//    }
    scad.close();
}
