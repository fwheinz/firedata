...
RList pmreg::decompose_mreg (RList *mreg, int steps, vector<RList>& mregs) {
...
vector<PMRegion> pmreg::decompose_pmreg (PMRegion *pmreg, int steps,
        vector<RList>& mregs) {
    ...
    if (steps > 0) {
        ...
        for (unsigned int i = 0; i < pmregs.size(); i++) {
            ...
            vector<RList> mregs;
            pmreg::decompose_mreg(&mreg, steps, mregs);
