...
void PMRegion::zthicknessprepare () {
	...
	zthicknesstree = new Tree(faces(zthicknesstmp).first, faces(zthicknesstmp).second, zthicknesstmp);
...
RList PMRegion::mincovered(Kernel::FT duration) {
 ...
 * (ccdpolyhedron) and returns the area, which is covered at most for
   the given duration                                                */
RList PMRegion::maxcovered(Kernel::FT duration) {
    return atinstant(duration);
}
...
#endif /* CGAL_VERSION_NR > 1041400000 */
