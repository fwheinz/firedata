...
public class Point implements Comparable {
    ...
    public boolean near(Point p) {
        if (MRGraphics.last == null)
            return false;
        Point p1 = MRGraphics.last.positionInBox(this);
        Point p2 = MRGraphics.last.positionInBox(p);
        return (p1.dist(p2) < 10);
    }
    ...
    public static Point random(BoundingBox bb) {
        double xs = bb.ur.x - bb.ll.x;
        double ys = bb.ur.y - bb.ll.y;

        double rx = r.nextDouble() * xs;
        double ry = r.nextDouble() * ys;
        Point ret = new Point(rx + bb.ll.x, ry + bb.ll.y);
